...
function renderFlashcard(){
  ...
  if(!flashState.flipped){
    body.innerHTML = '<div class="flashCardContainer">'
      ...
      +   ((ttsSupported() || VoiceClips.manifest) ? '<button class="flashSpeak" id="flashSpeakBtn">🔊 '+(lang==='es' ? 'Escuchar' : 'Listen')+'</button>' : '')
      + '</div>'
...
// Every fixed spoken line (civics Q&A, N-400 questions, officer feedback) ships
// as a studio-quality MP3 in audio/tts/, generated by scripts/generate-tts.js.
// Lookup is by normalized "lang|text" key via manifest.json. Anything not in
// the manifest (user-specific N-400 answers, Cami replies) — and any playback
// failure — falls back to the legacy speechSynthesis path, so this layer can
// never make speech WORSE than before.
var VoiceClips = {
  base: 'audio/tts/',
  manifest: null,   // key → filename, null until loaded (or load failed)
  player: null,     // single reusable Audio element, unlocked by first user gesture (iOS)
  active: null      // Audio element currently playing
};

// Must match clipKey() in scripts/generate-tts.js exactly.
function clipKey(langCode, text){
  return langCode + '|' + String(text || '').replace(/\s+/g, ' ').trim().toLowerCase();
}

(function loadVoiceClipManifest(){
  if(typeof fetch !== 'function') return;
  try {
    fetch(VoiceClips.base + 'manifest.json', {cache: 'force-cache'})
      .then(function(r){ return r.ok ? r.json() : null; })
      .then(function(m){ if(m && m.clips) VoiceClips.manifest = m.clips; })
      .catch(function(){ /* no clips shipped — speechSynthesis handles everything */ });
  } catch(e){}
})();

function clipUrlFor(langCode, text){
  if(!VoiceClips.manifest) return null;
  var f = VoiceClips.manifest[clipKey(langCode, text)];
  return f ? (VoiceClips.base + f) : null;
}

// iOS/WKWebView only lets audio start from a user gesture. Playing a silent
// clip on the first tap "unlocks" one Audio element we then reuse for every
// officer line, even ones triggered from timers.
var SILENT_WAV = 'data:audio/wav;base64,UklGRigAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQQAAAAAAA==';
function unlockVoiceClipPlayer(){
  if(VoiceClips.player) return;
  try {
    ...
    setTimeout(function(){ try { a.pause(); a.volume = 1; } catch(e){} }, 80);
    VoiceClips.player = a;
  } catch(e){}
}

// Nulling the handlers before pausing is also what cancels an in-flight clip
// SEQUENCE: the continuation lives in onended, so it simply never fires.
function stopVoiceClip(){
  var a = VoiceClips.active;
  if(!a) return;
  try { a.onended = null; a.onerror = null; a.pause(); } catch(e){}
  VoiceClips.active = null;
}

// Play one clip. Calls onEnd() when finished; calls onFail() (instead of onEnd)
// if playback can't start — caller falls back to speechSynthesis.
function playVoiceClip(url, opts, onEnd, onFail){
  try {
    stopVoiceClip();
    if(ttsSupported()) try { window.speechSynthesis.cancel(); } catch(e){}
    var a = VoiceClips.player || new Audio();
    VoiceClips.active = a;
    a.src = url;
    a.volume = 1.0;
    a.playbackRate = (opts && opts.playbackRate) || 1.0;
    // Slowed playback must not drop pitch (chipmunk in reverse) — keep it natural
    ...
    function finish(ok){
      ...
      if(VoiceClips.active === a) VoiceClips.active = null;
      if(ok){ if(onEnd) onEnd(); } else { if(onFail) onFail(); }
    }
    ...
    if(onFail) onFail();
    return true; // we already routed to onFail — caller must not double-speak
  }
}

// Play clips back-to-back (e.g. "The correct answer is:" + the answer).
// Routes to onFail only if the FIRST clip can't start (caller then falls back
// to full speechSynthesis); a later clip failing just ends the sequence —
// re-speaking the whole line would repeat what was already heard.
function playVoiceClipSequence(urls, opts, onEnd, onFail){
  function next(i){
    if(i >= urls.length){ if(onEnd) onEnd(); return; }
    playVoiceClip(urls[i], opts, function(){ next(i + 1); }, i === 0 ? onFail : onEnd);
  }
  next(0);
}

function speakInterview(text){
  // Pre-generated ElevenLabs clip first (same voice for EN and ES) …
  var url = clipUrlFor(lang === 'es' ? 'es' : 'en', text);
  if(url){
    playVoiceClip(url, null, null, function(){ speakInterviewLegacy(text); });
    return;
  }
  speakInterviewLegacy(text);
}

// … device speechSynthesis as the fallback for unclipped text or playback failure.
function speakInterviewLegacy(text){
...
function speakOfficerInternal(text, opts, onEnd){
  // Pre-generated clip first. The "slow" pronunciation variant reuses the same
  // clip at 0.8x with pitch preserved instead of shipping a second recording.
  var url = clipUrlFor('en', text);
  if(url){
    var slow = opts && opts.rate && opts.rate < 0.9;
    playVoiceClip(url, { playbackRate: slow ? 0.8 : 1.0 }, onEnd,
      function(){ speakOfficerLegacy(text, opts, onEnd); });
    return;
  }
  speakOfficerLegacy(text, opts, onEnd);
}
...
function processIntvAnswer(){
  ...
  // Officer responds
  setTimeout(function(){
    speakOfficerFeedback(result.verdict, variants[0], function(){
...
// Speaks the verdict. For close/wrong verdicts the line is "<stem> <expected>",
// where expected can be a fixed civics answer (has a clip) — play stem clip +
// answer clip back-to-back. User-specific N-400 answers have no clip, so the
// whole composed line falls back to speechSynthesis.
function speakOfficerFeedback(verdict, expected, onEnd){
  if(verdict !== 'correct'){
    var stem = (verdict === 'close') ? 'Close enough. The expected answer is:' : 'The correct answer is:';
    var stemUrl = clipUrlFor('en', stem);
    var ansUrl = clipUrlFor('en', expected);
    if(stemUrl && ansUrl){
      playVoiceClipSequence([stemUrl, ansUrl], null, onEnd,
        function(){ speakOfficerLegacy(officerFeedbackText(verdict, expected), null, onEnd); });
      return;
    }
  }
  speakAsOfficer(officerFeedbackText(verdict, expected), onEnd);
}
